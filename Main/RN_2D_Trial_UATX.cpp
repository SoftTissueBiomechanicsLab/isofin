...
{    
    ...
    InputFileDir="C://Users//mjl3967//Documents//Projects//project//isofin_results//Input//";// directory where mesh data and input file is stored.
    MeshFileDir="C://Users//mjl3967//Documents//Projects//project//isofin_results//Input//MeshData//";// directory where mesh data and input file is stored.
	ResultFileDir="C://Users//mjl3967//Documents//Projects//project//isofin_results////Results//"; // directory where result data and input file is stored.
	// Scalars
	double Max_Disp;
  ...
  //// Dof's for fibers and arcs and bending strips ////
	...
	while(disp_ratio < 1.0)
    {
      ...
      ngp=order+1;
      tie(d_DOF,F_DOF) = RN_2D_BC( num_cpt,Cpt,Face_D,Face_F,u0,disp_ratio ); // applied boundary conditions on a unit cube.
...
//		  tie(K,R,Q)=Network_KR_Parallel_Debug(P,Q,Patch,Patch_Bs,A0,A0_Bs,Mat,Mat_Arcs,Mat_Bs,DOF,d_DOF,F_DOF,u0,num_threads);
		  if(num_iter==1)
	       ...
	       // Calculate norm of the residue.
	       for (int j=0;j<Q.cols();j++)
	        {   
		        R_disp(3*j,0)=R(DOF(j,0)-1,0);R_disp(3*j+1,0)=R(DOF(j,1)-1,0);
				R_disp(3*j+2,0)=R(DOF(j,2)-1,0); R_rot(j,0)=R(DOF(j,3)-1,0); 
			}
//			cout << endl << " R_disp " << R_disp << endl << endl;
//			cout << endl << " R_rot " << R_rot << endl << endl;
            R_disp=R_disp/(Area*E_Bs);R_rot=R_rot/(Ip*E_Bs/r);
			Norm_R(0,num_iter-1)=R_disp.norm();Norm_R(1,num_iter-1)=R_rot.norm();
			// if (num_iter>2)
			// {
				// conv_order_disp=log(Norm_R(0,num_iter-1)/Norm_R(0,num_iter-2))/log(Norm_R(0,num_iter-2)/Norm_R(0,num_iter-3));
				// conv_order_rot=log(Norm_R(1,num_iter-1)/Norm_R(1,num_iter-2))/log(Norm_R(1,num_iter-2)/Norm_R(1,num_iter-3));
				// cout << endl << " conv_order_disp " << conv_order_disp << endl << endl;
			    // cout << endl << " conv_order_rot " << conv_order_rot << endl << endl;
			// }
//			cout << endl << " R_disp " << R_disp << endl << endl;
//			cout << endl << " R_rot " << R_rot << endl << endl;
//			cout << endl << " conv_order_disp " << conv_order_disp << endl << endl;
          ...
          if(num_iter>1)
            {
            	...
            	Stop_Key = Stop_Criterion_Network_2D(P,Q_pre,Q,R,Fluxes,d,DOF,d_DOF,u0,tol_D,tol_R,choice);
			}
