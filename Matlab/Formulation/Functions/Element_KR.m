function [ K_Element,F_Element ] = Element_KR(P,Q,End_pt,Ele2Cp,knotVector,order,weights,Mat,A0,ngp)
...
%     s = 0.5*((Ept2-Ept1)*Gauss_e(i)+(Ept2+Ept1));% s is Gauss pt in parametric space.
%     [N(i,:),dN1(i,:),dN2(i,:)]=NURBS1DBasis2ndDers(s,order,knotVector,weights);
%     K_temp=zeros(size(Ele2Cp,2)*4);F_temp=zeros(size(Ele2Cp,2)*4,1);
    for j=1:size(Ele2Cp,2)
        for j1=1:4
            ...
            % Normalized tangent vectors.
              ...
              tr=tangent_dr(a1(1:3,1),a1r(1:3,1),norm_a1);t1r=tangent_d1r(a1(1:3,1),a11(1:3,1),a1r(1:3,1),a11r(1:3,1),norm_a1);                
            % Required matrices.      
            ...
            %% Total element residue vector.  
              F_Element(r,1)=F_Element(r,1)+(Fm+Fb+Ft)*J1*Gauss_W(i);
              for k=1:size(Ele2Cp,2)
                for k1=1:4
                    ...
                    if r<=s                      
                        ...
                        as(k1,1)=N(1,k);a1s(k1,1)=dN1(i,k);a11s(k1,1)=dN2(i,k);psi_s=as(4,1);psi_1s=a1s(4,1); 
                        % Normalized tangent vectors.
