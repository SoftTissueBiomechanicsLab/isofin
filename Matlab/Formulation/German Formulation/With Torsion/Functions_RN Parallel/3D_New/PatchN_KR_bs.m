function [ K_patch,R_patch ] = PatchN_KR_bs(Nele,P,Qf,End_pt,Ele2Cp,knotVector,order,weights,Mat,DOF,A0,ngp)
...
for i=1:Nele %% MAKE PARALLEL HERE
    ...
    [ K_ele,R_ele ]=Element_KR_bs(P,Qf,End_pt(i,:),Ele2Cp(i,:),knotVector,order,weights,Mat,A0,ngp);
    ...
    K_patch=K_patch+sparse(row_index_K,column_index_K,K_ele,4*nos,4*nos);R_patch=R_patch+sparse(row_index_R,column_index_R,R_ele,4*nos,1);
end
end
