function [ K,R,Q ] = Network_KR(P,Q,Patch,Patch_Bs,A0,A0_Bs,Mat,Mat_Arcs,Mat_Bs,DOF,DOF_Bs,D_DOF,F_DOF)
%% Code
...
P_cell={};Q_cell={};A0_cell={};
for i=1:size_Patch(2)
    n=Patch(i).num_cpt;
    P_cell{i}=P(:,n);Q_cell{i}=Q(:,n);
    A0_cell{i}=A0(:,3*(i-1)+1:3*(i-1)+3);
end
parfor i=1:size_Patch(2) %% PARFOR LOOP
    ...
    nos=size(n,2);Nele=nos-order;p=P_cell{i};q=Q_cell{i};
    End_pt=zeros(Nele,2);
    for j=1:Nele
    ...
    a0=transpose(A0_cell{i});ngp=order+1; % We need transpose because mesh generator stores triad unit vectors in rows.
    if i<=(size_Patch(2)-size_Patch_Bs(2))% Distinguishing Between Arcs and fibers.
%         disp('Stiffness Contribution from fiber')
         [ K_patch,R_patch ] = PatchN_KR(Nele,p,q,End_pt,Ele2Cp,kv,order,wts,Mat,dof,a0,ngp);
    else
%         disp('Stiffness Contribution from arc')
        [ K_patch,R_patch ] = PatchN_KR(Nele,p,q,End_pt,Ele2Cp,kv,order,wts,Mat_Arcs,dof,a0,ngp);
    end
    
    %% Add K-patch and R_patch to the respective cells
    K_cell{1,i}=K_patch;R_cell{1,i}=R_patch;
end
size_Patch=size(Patch);size_Patch_Bs=size(Patch_Bs);
...
% Get K_patch and R_patch for ith patch.
   K_patch=K_cell{1,i};R_patch=R_cell{1,i};
   K_patch=full(K_patch);R_patch=full(R_patch);
   % Add to K and R
    for r=1:nos
        for r1=1:4
            a=n(1,r);
...
P_cell={};Q_cell={};A0_bs_cell={};
for i=1:size_Patch_Bs(2)
    n=Patch_Bs(i).num_cpt;
    P_cell{i}=P(:,n);Q_cell{i}=Q(:,n);
    A0_bs_cell{i}=A0_Bs(:,3*(i-1)+1:3*(i-1)+3);
end
parfor i=1:size_Patch_Bs(2) %% PARFOR LOOP
    ...
    a0=transpose( A0_bs_cell{i});ngp=order+1;
    [ K_bs,R_bs ] = PatchN_KR_bs(Nele,p,q,End_pt,Ele2Cp,kv,order,wts,Mat_Bs,dof,a0,ngp);
    %% Add K_bs and R_bs to the respective cells
    K_cell{1,i}=K_bs;R_cell{1,i}=R_bs;
end
...
% Get K_patch and R_patch for ith patch.
   K_bs=K_cell{1,i};R_bs=R_cell{1,i};
   K_bs=full(K_bs);R_bs=full(R_bs);
   % Add to K and R
    for r=1:nos
        for r1=1:4
            a=n(1,r);
