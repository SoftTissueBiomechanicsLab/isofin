function [ R ] = PatchN_R_bs(Nele,P,Qf,End_pt,Ele2Cp,knotVector,order,weights,Mat,DOF,A0,ngp)
...
% % %         if D_DOF(i,j+1)~=Inf
% % %         % Modify Q
% % %            Qf(j,D_DOF(i,1))=P(j,D_DOF(i,1))+D_DOF(i,j+1);
% % %         end
% % %     end
% % % end
...
nos=size(P,2);
% % K=zeros(nos*4,nos*4);
R=zeros(nos*4,1);% Only unconstrained dof are considered.
%% Apply residual forces before displacement/force increment.
for i=1:Nele 
  [ R_ele ]=Element_KR_bs(P,Qf,End_pt(i,:),Ele2Cp(i,:),knotVector,order,weights,Mat,A0,ngp);
  for j=1:size(Ele2Cp(i,:),2)
      for j1=1:4
          r=4*(j-1)+j1;% rth dof (local).
          R(DOF(Ele2Cp(i,j),j1),1)=R(DOF(Ele2Cp(i,j),j1),1)+R_ele(r,1);
% %           for k=1:size(Ele2Cp(i,:),2)
% %              for k1=1:4
% %                  s=4*(k-1)+k1;% sth dof (local).
% %                  if r<=s
% %                     a=DOF(Ele2Cp(i,j),j1);b=DOF(Ele2Cp(i,k),k1);
% %                      K(a,b)=K(a,b)+K_ele(r,s);
% %                      if a~=b
% %                         K(b,a)=K(b,a)+K_ele(r,s);
% %                      end
% %                  end
% %              end
% %          end
