function [ R,Q,Reaction ] = Build_R(Nele,P,Q,End_pt,Ele2Cp,knotVector,order,weights,Mat,DOF,A0,ngp,D_DOF,F_DOF)
...
% nos=size(P,2);
% K=zeros(NOS_Total*4,NOS_Total*4);
R=zeros(NOS_Total*4,1);% Only unconstrained dof are considered.
%% Apply residual forces before displacement/force increment.
parfor i=1:Nele % Parallel for loop here.
%   K_temp=zeros(NOS_Total*4,NOS_Total*4);
  R_temp=zeros(NOS_Total*4,1);% Only unconstrained dof are considered.
