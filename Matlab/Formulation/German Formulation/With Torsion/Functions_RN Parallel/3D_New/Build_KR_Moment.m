function [ K_nc,R_nc,t,f ] = Build_KR_Moment(Q,Ele2Cp,num_ele,NOS_Total,num_cp,kv,order,w,s,mag,angle,DOF)
%% Build_KR_Moment: Gives Stiffness(Global) matrix component due to point moment at the end of the cantilevered beam.
% Moment is applied by applying a couple at the last two control
% points.These control points are very close.
...
K_nc=zeros(NOS_Total*4);R_nc=zeros(NOS_Total*4,1);% Only unconstrained dof are considered.
%% CODE
...
R_nc(DOF(num_cp-1,1):DOF(num_cp-1,2),1)=-f;
force_mag1=zeros(order+1,1);force_mag2=zeros(order+1,1);
force_mag1(order+1,1)=mag;
force_mag2(order,1)=-mag;
%% Calculate tangent stiffness matrix due to the non-conservative load.
% Form K_temp and find contribution to K_nc.
K_temp1=zeros(nos*4);K_temp2=zeros(nos*4);
nor_a1=norm(a1);
for m=1:nos
    for m1=1:2
        for n=1:nos
            for n1=1:2
                a_temp=Rot*a1(1:2,:);
                K_temp1((m-1)*4+m1,(n-1)*4+n1)=force_mag1(m)*((dN1(n)*Rot(m1,n1)/nor_a1)-a_temp(m1,1)*dN1(n)*a1(n1,1)/nor_a1^3);
                K_temp2((m-1)*4+m1,(n-1)*4+n1)=force_mag2(m)*((dN1(n)*Rot(m1,n1)/nor_a1)-a_temp(m1,1)*dN1(n)*a1(n1,1)/nor_a1^3);
                K_nc(DOF(Ele2Cp(num_ele,m),m1),DOF(Ele2Cp(num_ele,n),n1))=K_nc(DOF(Ele2Cp(num_ele,m),m1),DOF(Ele2Cp(num_ele,n),n1))+K_temp1((m-1)*4+m1,(n-1)*4+n1);
                K_nc(DOF(Ele2Cp(num_ele,m),m1),DOF(Ele2Cp(num_ele,n),n1))=K_nc(DOF(Ele2Cp(num_ele,m),m1),DOF(Ele2Cp(num_ele,n),n1))+K_temp2((m-1)*4+m1,(n-1)*4+n1);
            end
        end
    end
end
