...
%% Inputs
% P: cpt matrix containing x-y-z of all cpts of undeformed configuration.
% Each column of P has one cpt 
% Q: cpt matrix containing x-y-z of all cpts at some newton iteration.
% Each column of Q has one cpt
% End_pt: row vector containing end points of an element.#
% Ele2Cp: a row vector containing index of control point; 
% control point indices are same as shape fun indices.
% dof: matrix with degrees of freedom corresponding to the control points.
...
% A0: Reference triad vectors. (3x3 matrix of those vectors).
% T0: Initial
%%
T0=A0(:,1)/dot_p(A0(:,1),A0(:,1))^0.5; % tangent vector to the centreline in refrence triad.
% Extract material/structural properties
Area=Mat(1); E=Mat(2); G=Mat(3); I2=Mat(4); I3=Mat(5); Ip=Mat(6);
%% Form element control point matrix.
P_ele=zeros(4,size(Ele2Cp,2));
Q_ele=P_ele;
for i=1:size(Ele2Cp,2)
    % Reference matrix containing x-y-z of all control points within element.
    P_ele(:,i)=P(:,Ele2Cp(1,i)); 
    % Current matrix containing x-y-z of all control points within element.
...
% Initialize N and dN's as zero matrices to be filled in.
N=zeros(ngp,nos); dN1=N; dN2=N;
Ept2=End_pt(1,2); Ept1=End_pt(1,1);
% Jacobian for parent-parameter pair. Note: For 1D no need of determinant.
J1=0.5*(End_pt(1,2)-End_pt(1,1));
for i=1:ngp
    s = 0.5*((Ept2-Ept1)*Gauss_e(i)+(Ept2+Ept1));% s is Gauss pt in parametric space.
    % Call Mex function to compute shape functions and derivatives
    [N(i,:),dN1(i,:),dN2(i,:)]=NURBS1DBasis2ndDers(s,order,knotVector,weights);
end
% Initialize energy scalars.
...
for i=1:ngp % Gauss integration
    ...
    for m1=1:nos
      xc=xc+N(i,m1)*Q_temp(:,m1);
      Xc=Xc+N_temp(i,m1)*P_temp(:,m1);
      a1=a1+dN1_temp(i,m1)*Q_temp(:,m1);
      A1=A1+dN1_temp(i,m1)*P_temp(:,m1);
      a11=a11+dN2_temp(i,m1)*Q_temp(:,m1);
      A11=A11+dN2_temp(i,m1)*P_temp(:,m1);
    end
    ...
    LT0T=Lambda(T0,T); LTt=Lambda(T,t); 
