...
for Case= 2   
    ...
    % Bool for if using voronoi network
    voronoi = 1;
...
for power=4
    ...
    elseif type==3
        ...
        % load network geometry
        filename = ['../Networks/Data/2D/',num2str(fiber_num),'/Case',num2str(Case)];
        load(filename)        
    ...
    switch deformation_type
        case 'SSX'        
            Face_D(1,:)=[0,Max_Disp,0,0]; % Face 1 (x=1)
            Face_D(2,:)=[0,0,0,0]; % Face 2 (x=0)
            Face_D(3,:)=[u0,u0,u0,u0]; % Face 3 (y=1)
            Face_D(4,:)=[u0,u0,u0,u0]; % Face 4 (y=0)
            Face_D(5,:)=[u0,u0,u0,u0]; % Face 5 (z=1)
            Face_D(6,:)=[u0,u0,u0,u0]; % Face 6 (z=0)
        case 'UAX'        
        ...
        case 'BIAX'
            Face_D(1,:)=[Max_Disp,0,0,0]; % Face 1 (x=1)
            Face_D(2,:)=[0,0,0,0]; % Face 2 (x=0)
            Face_D(3,:)=[0,Max_Disp,0,0]; % Face 3 (y=1)
            Face_D(4,:)=[0,0,0,0]; % Face 4 (y=0)
            Face_D(5,:)=[u0,u0,u0,u0]; % Face 5 (z=1)
            Face_D(6,:)=[u0,u0,u0,u0]; % Face 6 (z=0)
        case 'PS'            
            Face_D(1,:)=[Max_Disp,0,0,0]; % Face 1 (x=1)
            ...
            Face_D(3,:)=[u0,u0,u0,u0]; % Face 3 (y=1)
            Face_D(4,:)=[u0,u0,u0,u0]; % Face 4 (y=0)
            Face_D(5,:)=[u0,u0,0,0]; % Face 5 (z=1)
            Face_D(6,:)=[u0,u0,0,0]; % Face 6 (z=0)                    
    end
...
% Weights
fprintf(fileID,'%f ',Wts);
fprintf(fileID,'\r\n');
% Patch
fprintf(fileID,'%d ',size(Patch,2));
fprintf(fileID,'\r\n');
for i = 1:size(Patch,2)
    fprintf(fileID,'%d %d %d ',size(Patch(i).kv,2),size(Patch(i).wts,2),Patch(i).order);
    fprintf(fileID,'\r\n');

    fprintf(fileID,'%f ',Patch(i).kv);
    fprintf(fileID,'\r\n');

    fprintf(fileID,'%f ',Patch(i).wts);
    fprintf(fileID,'\r\n');

    fprintf(fileID,'%d ',Patch(i).num_cpt);
    fprintf(fileID,'\r\n');
