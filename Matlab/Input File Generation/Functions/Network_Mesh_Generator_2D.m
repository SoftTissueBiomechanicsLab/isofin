function [ Cpt,Wts,Patch,Patch_Bs,A0,A0_Bs ] = Network_Mesh_Generator_2D( vx,vy,ele_size,order,end_cpt_factor,trim_factor,fig )
...
%     kv(1,order+2)=ele_size/10;kv(1,size(kv,2)-order-1)=1-ele_size/10;
    ...
    cpt(1:2,2)=p1+(l*end_cpt_factor)*(p2-p1)/l;cpt(1:2,nos-1)=p2+(l*end_cpt_factor)*(p1-p2)/l;
    for j=1:nos-4
    ...
    if fig=='on'
        plot(cpt(1,:),cpt(2,:),'r.','MarkerSize',15);hold on;
    end
...
for i=1:size(int_pt,1)
    ...
    if fig=='on'
        plot(cpt_arc(1,:),cpt_arc(2,:),'k.','MarkerSize',15);hold on
    end
    %% Define arcs
    for j=1:3
...
if fig=='on'
    ...
    for i=size(vx,2)+1:size(Patch,2)
        ...
        plot3(L0(:,1),L0(:,2),L0(:,3),'g');hold on
    end
end
end