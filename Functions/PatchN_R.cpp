...
// to be oontinued ......
tuple<MatrixXd>PatchN_R(int &Nele,MatrixXd &P,MatrixXd &Q,MatrixXd &End_pt,MatrixXi &Ele2Cp,RowVectorXd &knotVector,int &order,RowVectorXd &weights,RowVectorXd &Mat,MatrixXi &DOF,Matrix3d &A0,int &ngp)
{ 
	...
	MatrixXd R_patch(num_dof,1),R_ele(4*nos,1);; // initializing element K(consistent tangent),R and patch R (DENSE MATRIX).
	R_patch = MatrixXd::Zero(num_dof,1);
	R_ele = MatrixXd::Zero(4*nos,1);
	...
	{   
		...
		tie(R_ele)=Element_R(P,Q,vec1,vec2,knotVector,order,weights,vec3,A0,ngp);
		for ( j=0;j<nos;j++ )
		{   
			for ( j1=0;j1<dof;j1++ )
		    {   
